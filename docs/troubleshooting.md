...
| --- | --- | --- |
| `polepos: command not found` | Not installed, or venv not active | [`polepos` Is Not Found](#polepos-is-not-found) |
| `check` says "not a project" | Run outside the project, or no manifest | [Not a Project](#polepos-check-says-this-is-not-a-project) |
| `check` reports a missing managed marker | A managed insertion point was edited out | [Missing Managed Marker](#polepos-check-reports-a-missing-managed-marker) |
| `add module` fails before writing files | Preflight found drift or a name clash | [add module Fails](#polepos-add-module-fails-before-writing-files) |
| `remove module` fails before deleting | Preflight found hand-edited or drifted wiring | [remove module Fails](#polepos-remove-module-fails-before-deleting-files) |
| Pytest fails after editing schemas | Generated tests assert the old shape | [Pytest Fails](#pytest-fails-after-editing-module-schemas) |
| Integration env exists but `check` says missing | Value is commented out or empty | [Integration Env](#integration-env-exists-but-check-says-it-is-missing) |
| A removed module's table still exists | Removal does not drop database tables | [Removed Table](#a-removed-modules-table-still-exists) |
| Migrations cannot connect | `DATABASE_URL` or the database is unavailable | [Migrations Cannot Connect](#database-migrations-cannot-connect) |
| A new model is not autogenerated | Model not imported into the metadata | [Autogenerate](#a-new-model-is-not-included-in-alembic-autogenerate) |
| CORS or runtime settings do not change | Settings cached, or `.env` not reloaded | [Settings Do Not Change](#cors-or-runtime-settings-do-not-change) |
